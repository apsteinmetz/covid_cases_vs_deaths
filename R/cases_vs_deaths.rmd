...
I chose to add sixty days of leads. I don't really think that long a lead is realistic and, given the pandemic has been around only nine months, there aren't as many data points sixty days ahead.  Still, I want to see the behavior of the models.  Once we have created the leads we remove any dates for which we don't have led deaths.
...
```
## Make Predictions
Let's use `predict` to see how well our model fits to the actual deaths.
...
The best fit still seems to systematically under- and over-estimate the effect of time even though the predictions match up pretty well with the dates of peaks and troughs in fatality.  At the risk of overfitting, we'll see if making time a nonlinear variable is a good solution.
...
4. `map` each set to a regression model.
...
To see how the fit looks for the data set as a whole we look at a histogram of all the state R-squareds.  We see most of the state models have a slightly worse accuracy, on average, than the national model.
```{r message=FALSE, warning=FALSE}
...
Unfortunately, there are vast differences in the bet fit lead times across the states.  The median lead time is `r median(best_fit_st$lead)` days but most are shorter or longer.
```{r}
